...
from app.handlers.handlers import RabbitDbConsumer, RabbitTgConsumer, RedisRabbitConsumer, TgRedisConsumer
...
class Factory:
    ...
    async def run_tg_consumers(self) -> None:
        ...
        tg_redis_consumer = TgRedisConsumer(self._create_chunks_storage(), bot)
        task1 = asyncio.create_task(rabbit_tg_consumer.run())
        task2 = asyncio.create_task(tg_redis_consumer.run())
        await asyncio.gather(task1, task2)

    async def run_redis_rabbit_consumer(self) -> None:
        redis_rabbit_consumer = RedisRabbitConsumer(self._create_chunks_storage(), await self._create_rabbit())
        await redis_rabbit_consumer.run()
    ...
    async def _create_rabbit(self) -> Rabbit:
        ...
        return Rabbit(channel)  # type: ignore[arg-type]
